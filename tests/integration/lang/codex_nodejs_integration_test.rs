...
/// Cargo test wrapper for Node.js Codex integration test
///
/// This wrapper invokes the shell script and validates output for code coverage.
#[tokio::test]
#[ignore]
async fn test_nodejs_codex_via_script() {
    println!("\n🚀 Node.js Codex Integration Test (via script)");
    println!("═══════════════════════════════════════════════");
    ...
    assert_eq!(results["test_run"]["language"].as_str(), Some("nodejs"));
    assert_eq!(results["test_run"]["ai_client"].as_str(), Some("codex"));

    println!("✅ Node.js Codex test completed");
}
