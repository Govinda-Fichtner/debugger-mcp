...
async fn test_fizzbuzz_debugging_integration() {
    ...
    let start_result = tools_handler.handle_tool("debugger_start", start_args).await;

    // If adapter spawn fails (e.g., debugpy not found), skip test gracefully
    ...
    if let Ok(contents) = session_details {
    ...
    let disconnect_result = tools_handler.handle_tool("debugger_disconnect", disconnect_args).await;

    if disconnect_result.is_ok() {
        println!("✅ Session disconnected successfully");
    } else {
        println!("⚠️  Disconnect may have issues: {:?}", disconnect_result);
    }
