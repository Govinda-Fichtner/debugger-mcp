...
# Check for debug gem
if ! ruby -e "require 'debug'" &> /dev/null; then
    echo "⚠️  Warning: debug gem not found (Codex may use its own)"
else
...
# 2. Verify Ruby fixture exists
echo "📝 Step 2: Verifying Ruby test fixture..."
echo "──────────────────────────────────────"
...
MCP_SERVER_NAME="debugger-test-ruby-simplified"
...
cat > "$PROMPT_FILE" << EOF
# Ruby Debugging Test with Codex

**IMPORTANT**: You have access to an MCP server called \`debugger-test-ruby-simplified\` that provides debugging tools.

**Step 1: First, list ALL available tools and resources from the \`debugger-test-ruby-simplified\` MCP server**
Use the list_mcp_tools MCP tool to show what debugging capabilities are available.

**Step 2: Then perform the debugging test:**
1. Use the debugger tools from \`debugger-test-ruby-simplified\` to start a debugging session for: $FIXTURE_SOURCE
