...
echo "🚀 Codex Ruby Integration Test (Simplified)"
echo "============================================"
echo ""
...
if ! ruby -e "require 'debug'" 2>/dev/null; then
    echo "❌ debug gem not found (required for Ruby debugging)"
    exit 1
fi
...
# 2. Check Ruby fixture (no compilation needed)
echo "📋 Step 2: Checking Ruby test fixture..."
echo "──────────────────────────────────────"

FIXTURE_SOURCE="$WORKSPACE_ROOT/tests/fixtures/fizzbuzz.rb"

if [ ! -f "$FIXTURE_SOURCE" ]; then
    echo "❌ Ruby fixture not found: $FIXTURE_SOURCE"
    exit 1
fi

echo "✅ Ruby fixture found: $FIXTURE_SOURCE"

# Verify syntax
if ! ruby -c "$FIXTURE_SOURCE" >/dev/null 2>&1; then
    echo "❌ Ruby syntax error in fixture"
    exit 1
fi
...
PROMPT_FILE="$WORKSPACE_ROOT/debug_prompt_ruby.md"

cat > "$PROMPT_FILE" << EOF
# Ruby Debugging Test with Codex - Enhanced Version

**IMPORTANT**: You have access to an MCP server called \`debugger-ruby\` that provides debugging tools.
...
Call \`list_mcp_resources\` on the \`debugger-ruby\` MCP server to discover:
...
{
  "sessionId": "<session-id>",
  "sourcePath": "/workspace/tests/fixtures/fizzbuzz.rb",
  "line": 2
}
\`\`\`
...
**Note**: Line 2 is the first if statement in the fizzbuzz method
...
**Verification**: Value should be 1 (Integer type)
**Context**: Variable 'n' is the parameter to the fizzbuzz method
...
\`\`\`markdown
1. \`list_mcp_resources\` (server="debugger-ruby"): <describe result>
2. \`list_mcp_resource_templates\` (server="debugger-ruby"): <describe result>
...
6. \`debugger_set_breakpoint\` (file "/workspace/tests/fixtures/fizzbuzz.rb", line 2): <describe result>
...
\`\`\`
list_mcp_resources(server="debugger-ruby")
list_mcp_resource_templates(server="debugger-ruby")
\`\`\`
...
**Why**: Launches the Ruby script under debugger control, paused at entry point.
...
**Why**: Line 2 is the first condition in fizzbuzz method.
...
\`\`\`
**Expected**: Array of frames, top frame at fizzbuzz.rb:2
...
- **Use exact paths**: The source path must be \`/workspace/tests/fixtures/fizzbuzz.rb\`
...
echo ""
echo "🎉 Codex Ruby integration test completed!"
