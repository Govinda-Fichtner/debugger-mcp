#!/bin/bash
# Simplified Codex Go Integration Test
# Runs Codex directly without Docker-in-Docker complexity

set -e

echo "🚀 Codex Go Integration Test (Simplified)"
echo "=========================================="
echo ""
...
if ! command -v dlv &> /dev/null; then
    echo "❌ dlv (Delve debugger) not found (required for Go debugging)"
    exit 1
fi
echo "✅ dlv (Delve) available"

echo ""

# 2. Compile Go fixture
echo "🔨 Step 2: Compiling Go test fixture..."
echo "──────────────────────────────────────"

FIXTURE_SOURCE="$WORKSPACE_ROOT/tests/fixtures/fizzbuzz.go"
OUTPUT_DIR="$WORKSPACE_ROOT/tests/fixtures/target"
FIZZBUZZ_BINARY="$OUTPUT_DIR/fizzbuzz-go"
...
go build -gcflags="all=-N -l" -o "$FIZZBUZZ_BINARY" "$FIXTURE_SOURCE"
...
if readelf -S "$FIZZBUZZ_BINARY" 2>/dev/null | grep -q ".debug_info"; then
...
PROMPT_FILE="$WORKSPACE_ROOT/debug_prompt_go.md"

cat > "$PROMPT_FILE" << EOF
# Go Debugging Test with Codex - Enhanced Version

**IMPORTANT**: You have access to an MCP server called \`debugger-go\` that provides debugging tools.
...
Call \`list_mcp_resources\` on the \`debugger-go\` MCP server to discover:
...
\`\`\`json
{
  ...
  "sourcePath": "/workspace/tests/fixtures/fizzbuzz.go",
  "line": 13
}
\`\`\`
...
**Verification**:
- Top frame should be \`main.fizzbuzz\` at line 13
...
\`\`\`markdown
1. \`list_mcp_resources\` (server="debugger-go"): <describe result>
2. \`list_mcp_resource_templates\` (server="debugger-go"): <describe result>
3. \`debugger_start\` (program "$FIZZBUZZ_BINARY", stopOnEntry=true): session <id> started
4. \`debugger_wait_for_stop\` (timeoutMs=5000): <describe result>
5. \`debugger_session_state\`: <describe result>
6. \`debugger_set_breakpoint\` (file "/workspace/tests/fixtures/fizzbuzz.go", line 13): <describe result>
...
\`\`\`
list_mcp_resources(server="debugger-go")
list_mcp_resource_templates(server="debugger-go")
\`\`\`
...
**Why**: Line 13 is the first condition in fizzbuzz function.
...
\`\`\`
**Expected**: Array of frames, top frame at fizzbuzz.go:13
...
- **Use exact paths**: The source path must be \`/workspace/tests/fixtures/fizzbuzz.go\`
- **Breakpoint line**: Line 13 is the first if statement checking \`n%15 == 0\`
- **Variable evaluation**: The variable 'n' should be 1 on first breakpoint hit
- **Frame names**: Go frames will be \`main.fizzbuzz\` and \`main.main\`
...
echo ""
echo "🎉 Codex Go integration test completed!"
