...
if ! command -v dlv &> /dev/null; then
    echo "❌ dlv (Delve debugger) not found"
    exit 1
fi
DLV_VERSION=$(dlv version 2>&1)
echo "✅ dlv: $DLV_VERSION"
...
# Compile with debug symbols and disable optimizations
# -gcflags="all=-N -l" disables inlining and optimizations for better debugging
go build \
    -gcflags="all=-N -l" \
    -o "$FIZZBUZZ_BINARY" \
    "$FIXTURE_SOURCE"
...
MCP_SERVER_NAME="debugger-test-go-simplified"
...
cat > "$PROMPT_FILE" << EOF
# Go Debugging Test with Codex

**IMPORTANT**: You have access to an MCP server called \`debugger-test-go-simplified\` that provides debugging tools.

**Step 1: First, list ALL available tools and resources from the \`debugger-test-go-simplified\` MCP server**
Use the list_mcp_tools MCP tool to show what debugging capabilities are available.

**Step 2: Then perform the debugging test:**
1. Use the debugger tools from \`debugger-test-go-simplified\` to start a debugging session for: $FIZZBUZZ_BINARY
