...
# Check for js-debug adapter
JSDEBUG_PATH="/tmp/js-debug"
if [ -d "$JSDEBUG_PATH" ]; then
    echo "✅ js-debug adapter available at: $JSDEBUG_PATH"
else
    echo "⚠️  Warning: js-debug not found at $JSDEBUG_PATH (Codex may use its own)"
fi
...
# 2. Verify Node.js fixture exists
echo "📝 Step 2: Verifying Node.js test fixture..."
echo "──────────────────────────────────────"
...
MCP_SERVER_NAME="debugger-test-nodejs-simplified"
...
cat > "$PROMPT_FILE" << EOF
# Node.js Debugging Test with Codex

**IMPORTANT**: You have access to an MCP server called \`debugger-test-nodejs-simplified\` that provides debugging tools.

**Step 1: First, list ALL available tools and resources from the \`debugger-test-nodejs-simplified\` MCP server**
Use the list_mcp_tools MCP tool to show what debugging capabilities are available.

**Step 2: Then perform the debugging test:**
1. Use the debugger tools from \`debugger-test-nodejs-simplified\` to start a debugging session for: $FIXTURE_SOURCE
