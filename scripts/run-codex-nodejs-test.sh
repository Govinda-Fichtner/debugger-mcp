#!/bin/bash
# Simplified Codex Node.js Integration Test
# Runs Codex directly without Docker-in-Docker complexity

set -e

echo "🚀 Codex Node.js Integration Test (Simplified)"
echo "==============================================="
echo ""
...
# Note: node-inspect is built into Node.js (no separate package needed)
echo "✅ node-inspect available (built-in)"

echo ""

# 2. Check Node.js fixture (no compilation needed)
echo "📋 Step 2: Checking Node.js test fixture..."
echo "──────────────────────────────────────"

FIXTURE_SOURCE="$WORKSPACE_ROOT/tests/fixtures/fizzbuzz.js"

if [ ! -f "$FIXTURE_SOURCE" ]; then
    echo "❌ Node.js fixture not found: $FIXTURE_SOURCE"
    exit 1
fi

echo "✅ Node.js fixture found: $FIXTURE_SOURCE"

# Verify syntax
if ! node --check "$FIXTURE_SOURCE" 2>/dev/null; then
    echo "❌ Node.js syntax error in fixture"
    exit 1
fi
echo "✅ Node.js syntax valid"
...
MCP_SERVER_NAME="debugger-nodejs"
...
PROMPT_FILE="$WORKSPACE_ROOT/debug_prompt_nodejs.md"

cat > "$PROMPT_FILE" << EOF
# Node.js Debugging Test with Codex - Enhanced Version

**IMPORTANT**: You have access to an MCP server called \`debugger-nodejs\` that provides debugging tools.
...
Call \`list_mcp_resources\` on the \`debugger-nodejs\` MCP server to discover:
...
**Verification**: Value should be 1 (number type)
...
\`\`\`markdown
1. \`list_mcp_resources\` (server="debugger-nodejs"): <describe result>
2. \`list_mcp_resource_templates\` (server="debugger-nodejs"): <describe result>
...
\`\`\`
list_mcp_resources(server="debugger-nodejs")
list_mcp_resource_templates(server="debugger-nodejs")
\`\`\`
...
**Why**: Launches the Node.js script under debugger control, paused at entry point.
...
echo ""
echo "🎉 Codex Node.js integration test completed!"
